...
func TestTaskCreateRefusesAgentKillingFirstCommand(t *testing.T) {
	...
	cases := []struct {
		...
		{name: "output redirected to a log still runs the stop first", script: "systemctl stop lattice-agent >/var/log/task.log 2>&1", refused: true},
		{name: "a trailing comment does not change what runs", script: "systemctl restart lattice-agent  # bounce first", refused: true},
		{name: "a flag after the unit leaves the agent just as dead", script: "systemctl restart lattice-agent --no-block", refused: true},
		{name: "a unit that merely starts with the name is allowed", script: "systemctl restart lattice-agent-exporter"},
